...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
      ...
      _timer = Timer.periodic(
        ...
        (Timer timer) async {
          ...
          if (_seekPosition != null && newPosition != null) {
            ...
            // Crinza patch: also release when the playhead has settled
            // NEAR the target, not only past it. A live forward seek gets
            // clamped to the seekable end, so `difference` can stay
            // negative for ever and latch the flag on — freezing the
            // scrubber's position readout for the rest of the session.
            if (difference > 0 || difference.abs() < 1000) {
              _seekPosition = null;
            }
          }
        },
  ...
  /// Crinza patch: how far behind the live edge this player currently
  /// sits, in milliseconds. -1 when not live / not yet known.
  Future<int> get liveOffsetMs async {
    if (!value.initialized && _isDisposed) return -1;
    return _videoPlayerPlatform.getLiveOffset(_textureId);
  }

  /// Crinza patch: place the playhead [offsetMs] behind the live edge,
  /// with the edge read natively at seek time. Forward-only.
  Future<int> seekToLiveOffset(int offsetMs) async {
    if (!value.initialized || _isDisposed) return -1;
    return _videoPlayerPlatform.seekToLiveOffset(_textureId, offsetMs);
  }

  /// Crinza patch: raw live timeline numbers for latency diagnosis.
  Future<Map<String, dynamic>> get liveDebugInfo async {
    if (!value.initialized || _isDisposed) return <String, dynamic>{};
    return _videoPlayerPlatform.getLiveDebugInfo(_textureId);
  }
  ...
  void _updatePosition(Duration? position, {DateTime? absolutePosition}) {
    ...
    // Crinza patch: absolutePosition is updated UNCONDITIONALLY.
    //
    // It used to be gated on `_seekPosition == null`, which froze it for
    // good on a live stream. `_seekPosition` holds the *requested* target
    // and is only cleared once the playhead passes it:
    //
    //     if (newPosition - _seekPosition > 0) _seekPosition = null;
    //
    // A forward seek on a live source is clamped by the platform to the
    // seekable end, which can be BEFORE the request — so the playhead
    // never passes it and the flag is never cleared. Every later tick
    // then skipped this line, `absolutePosition` stopped advancing, and
    // the on-screen stream timer froze permanently while the video kept
    // playing normally. Tapping GO LIVE reproduced it every time.
    //
    // Suppressing `position` during a seek is deliberate (it stops the
    // scrubber jumping about). `absolutePosition` is an absolute
    // wall-clock stamp read fresh from the platform each tick and needs
    // no such smoothing.
