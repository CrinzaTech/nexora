...
abstract class VideoPlayerPlatform {
  ...
  ///Crinza patch: how far behind the live edge the player currently sits,
  ///in milliseconds, straight from the platform player. -1 when the
  ///stream is not live or the offset is not known yet.
  Future<int> getLiveOffset(int? textureId) {
    throw UnimplementedError('getLiveOffset() has not been implemented.');
  }

  ///Crinza patch: seek to [offsetMs] behind the live edge, with the edge
  ///read natively at seek time (the Dart-side duration is a stale
  ///snapshot). Returns the resulting offset, or -1 if not applicable.
  Future<int> seekToLiveOffset(int? textureId, int offsetMs) {
    throw UnimplementedError('seekToLiveOffset() has not been implemented.');
  }

  ///Crinza patch: raw live timeline numbers, for splitting measured
  ///latency into player position vs upstream pipeline delay.
  Future<Map<String, dynamic>> getLiveDebugInfo(int? textureId) {
    throw UnimplementedError('getLiveDebugInfo() has not been implemented.');
  }
...
class DataSource {
  ...
  /// Live streams: target distance behind the live edge, in ms.
  final int? liveTargetOffsetMs;
