...
class BetterPlayerDataSource {
  ...
  ///Live streams only: how far behind the live edge the player should sit,
  ///in milliseconds. Android maps it to ExoPlayer's
  ///MediaItem.LiveConfiguration.targetOffsetMs (the player nudges playback
  ///speed to hold the offset); iOS to
  ///AVPlayerItem.configuredTimeOffsetFromLive with
  ///automaticallyPreservesTimeOffsetFromLive. Null keeps each platform's
  ///default behaviour. (Crinza patch — not part of upstream 1.0.8.)
  ...
  BetterPlayerDataSource copyWith({
    ...
    return BetterPlayerDataSource(
      ...
      liveTargetOffsetMs: liveTargetOffsetMs ?? this.liveTargetOffsetMs,
    );
  }
}
