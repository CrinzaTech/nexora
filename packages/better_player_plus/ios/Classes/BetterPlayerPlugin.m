...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([@"init" isEqualToString:call.method]) {
        ...
        if ([@"setDataSource" isEqualToString:call.method]) {
            ...
            int liveTargetOffsetMs = 0;
            id liveTargetObject = [dataSource objectForKey:@"liveTargetOffsetMs"];
            if (liveTargetObject != nil && liveTargetObject != [NSNull null]){
                liveTargetOffsetMs = [liveTargetObject intValue];
            }
            player.liveTargetOffsetMs = liveTargetOffsetMs;
        ...
        } else if ([@"seekToLiveOffset" isEqualToString:call.method]) {
            // Crinza patch: parity with Android.
            int64_t offsetMs = [argsMap[@"offsetMs"] longLongValue];
            result(@([player seekToLiveOffset:offsetMs]));
        } else if ([@"liveOffset" isEqualToString:call.method]) {
            // AVPlayer exposes no equivalent of getCurrentLiveOffset();
            // -1 is the documented "not known" value on both platforms.
            result(@(-1));
        } else if ([@"liveDebugInfo" isEqualToString:call.method]) {
            result(@{@"ok": @(NO)});
