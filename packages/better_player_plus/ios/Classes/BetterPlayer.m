...
- (void)setDataSourceURL:(NSURL*)url withKey:(NSString*)key withCertificateUrl:(NSString*)certificateUrl withLicenseUrl:(NSString*)licenseUrl withHeaders:(NSDictionary*)headers withCache:(BOOL)useCache cacheKey:(NSString*)cacheKey cacheManager:(CacheManager*)cacheManager overriddenDuration:(int) overriddenDuration videoExtension: (NSString*) videoExtension{
    ...
    if (@available(iOS 13.0, *)) {
        if (_liveTargetOffsetMs > 0) {
            // Crinza patch: keep every viewer the same distance behind the
            // live edge; AVPlayer preserves the offset across stalls.
            item.automaticallyPreservesTimeOffsetFromLive = YES;
            item.configuredTimeOffsetFromLive = CMTimeMakeWithSeconds(_liveTargetOffsetMs / 1000.0, 1000);
        }
    }
...
// Crinza patch: seek to exactly `offsetMs` behind the live edge, with the
// edge read from `seekableTimeRanges` at the moment of the seek.
//
// Mirrors BetterPlayer.kt#seekToLiveOffset. Same diagnostic codes:
//   -1 no player item, -2 no seekable range, -3 already closer than target.
- (int64_t)seekToLiveOffset:(int64_t)offsetMs {
    AVPlayerItem* item = _player.currentItem;
    if (item == nil) return -1;
    NSArray<NSValue*>* ranges = item.seekableTimeRanges;
    if (ranges.count == 0) return -2;
    CMTimeRange lastRange = [ranges.lastObject CMTimeRangeValue];
    CMTime edge = CMTimeRangeGetEnd(lastRange);
    if (!CMTIME_IS_VALID(edge)) return -2;
    Float64 edgeSec = CMTimeGetSeconds(edge);
    if (isnan(edgeSec) || edgeSec <= 0) return -2;
    Float64 targetSec = edgeSec - (offsetMs / 1000.0);
    if (targetSec < 0) targetSec = 0;
    Float64 currentSec = CMTimeGetSeconds([_player currentTime]);
    // Forward-only: a player already closer to live than the target is
    // where we want it, and yanking it back only adds latency.
    if (isnan(currentSec) || targetSec <= currentSec) return -3;
    [_player seekToTime:CMTimeMakeWithSeconds(targetSec, NSEC_PER_SEC)
        toleranceBefore:kCMTimeZero
         toleranceAfter:kCMTimeZero];
    return (int64_t)((edgeSec - targetSec) * 1000);
}
