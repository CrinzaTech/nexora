...
//                    )
    ...
    private fun onMethodCall(
        ...
        when (call.method) {
            ...
            // Crinza patch: live-offset diagnostic.
            LIVE_OFFSET_METHOD -> result.success(player.currentLiveOffset)
            LIVE_DEBUG_METHOD -> result.success(player.liveDebugInfo())
            // Crinza patch: place the playhead at a live offset, using a
            // live-edge reading taken natively at seek time.
            SEEK_TO_LIVE_OFFSET_METHOD -> {
                val offsetMs = (call.argument<Number>("offsetMs") ?: 0).toLong()
                result.success(player.seekToLiveOffset(offsetMs))
            }
            SET_SPEED_METHOD -> {
    ...
    private fun setDataSource(
        ...
        if (dataSource[ASSET_PARAMETER] != null) {
            ...
            val liveTargetOffsetNumber: Number =
                getParameter(dataSource, "liveTargetOffsetMs", 0)
            player.setDataSource(
                flutterState!!.applicationContext,
                key,
                uri,
    ...
    companion object {
        ...
        private const val LIVE_OFFSET_METHOD = "liveOffset"
        private const val LIVE_DEBUG_METHOD = "liveDebugInfo"
        private const val SEEK_TO_LIVE_OFFSET_METHOD = "seekToLiveOffset"
